...
!
!
#if ! (defined __NON_BLOCKING_SCATTER || defined __RMA_SCATTER)
!
...
!
#elif defined(__NON_BLOCKING_SCATTER)
!
...
SUBROUTINE fft_scatter ( dfft, f_in, nr3x, nxx_, f_aux, ncp_, npp_, isgn, dtgs )
  ...
  IF ( isgn .gt. 0 ) THEN
     ...
     DO proc = 1, nprocp
        ...
        !
        CALL MPI_GET(f_in ( (proc-1)*sendsiz + 1  ),  sendsiz, MPI_DOUBLE_COMPLEX, &
               proc-1, TARGET_DISP,  sendsiz, MPI_DOUBLE_COMPLEX, WIN, ierr)
        !
     ENDDO
     !
     ! maybe useless; ensures that no garbage is present in the output
